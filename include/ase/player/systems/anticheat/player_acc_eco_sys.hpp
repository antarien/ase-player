 ...
 *
 * @file        player_acc_eco_sys.hpp
 * @brief       PlayerAccEcoSystem - Engine-level anti-cheat economy-transaction detector
 * @description SERVER-ONLY authority system (Phase 13 AP-3): compares a player's realised economy
 *              transaction rate against the engine authority (types.hpp) and flags violations on the
 *              Hub as the exact contract topic "ECONOMY_TRANSACTION_*" (consumed by the EconomyAuditor
 ...
 *
 *   PlayerStaEcoComponent → PlayerAccEcoSystem → Hub "ECONOMY_TRANSACTION_*"
 *   (realised txn rate)     (rate vs authority)   (1.0 suspicious / 0.0 clear)
 *
...
public:
    const char* name() const override { return "PlayerAccEcoSystem"; }
