 ...
 *
 * @file        player_acc_cmb_sys.hpp
 * @brief       PlayerAccCmbSystem - Engine-level anti-cheat combat-rate detector
 * @description SERVER-ONLY authority system (Phase 13 AP-3): compares a player's realised combat-event
 *              rate against the engine authority (types.hpp) and flags violations on the Hub as the
 *              exact contract topic "COMBAT_EVENT_*" (consumed by the BehaviorWatcher skill; the
 *              literal `*` is part of the topic name per ANTI_CHEAT_COMPONENT_CONTRACT.md).
 *
 * @module      ase-player
 * @layer       3 (Modules)
 ...
 *
 *   PlayerStaCmbComponent → PlayerAccCmbSystem → Hub "COMBAT_EVENT_*"
 *   (realised combat rate)  (rate vs authority)   (1.0 suspicious / 0.0 clear)
 *
...
public:
    const char* name() const override { return "PlayerAccCmbSystem"; }
