 ...
 *
 * @file        player_acc_cmb_sys.cpp
 * @brief       PlayerAccCmbSystem - Engine-level anti-cheat combat-rate detector
 * @description SERVER-ONLY authority: flags a player whose realised combat-event rate exceeds the
 *              engine authority as the exact contract trigger "COMBAT_EVENT_*" (BehaviorWatcher).
 *
 * @module      ase-player
 * @layer       3 (Modules)
 ...
 * @version     1.0.0
 *
 * CAUSAL CHAIN (CAUSA_PLR_ACC_CMB: realised combat rate → anti-cheat flag)
 *
 *   [PlayerStaCmbComponent — realised combat rate after PlayerSimActSystem]
 *          │
 *          │ combat_rate
 *          ▼
 *   ┌──────────────────────────────────────────────────────────┐
 ...
 *   "COMBAT_EVENT_*"_hs → 1.0 when the realised combat rate exceeds the authority, else 0.0
 *
...
void PlayerAccCmbSystem::on_start(ecs::Registry& /*registry*/) {
    log::debug("[PlayerAccCmbSystem] Started");
}

void PlayerAccCmbSystem::tick(ecs::Registry& registry, float /*dt*/) {
    /**
     * Per player: compare the realised combat-event rate against the engine combat authority. A
     * realised rate above the bound cannot come from legitimate play — it is a combat automation
     * hack. Flag it on the Hub; the Replica forwards it as the exact BehaviorWatcher contract
    ...
    for (auto [entity, id, cmb] : view.each()) {
        ...
        const bool suspicious = cmb.combat_rate > PLR_AC_MAX_COMBAT_RATE;

        const bool was_suspicious =
            hub::exists(registry, owner, "COMBAT_EVENT_*"_hs) &&
            hub::get(registry, owner, "COMBAT_EVENT_*"_hs) >= 0.5f;
        hub::set(registry, owner, "COMBAT_EVENT_*"_hs, suspicious ? 1.0f : 0.0f);

        if (suspicious && !was_suspicious) {
            log::warn("[PlayerAccCmbSystem] player {} flagged (realised combat rate {} vs authority {}) → COMBAT_EVENT_*",
                      owner, cmb.combat_rate, PLR_AC_MAX_COMBAT_RATE);
        }
    }
}

void PlayerAccCmbSystem::on_stop(ecs::Registry& /*registry*/) {
    log::debug("[PlayerAccCmbSystem] Stopped");
}
